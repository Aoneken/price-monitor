...
CLEANING_PATTERNS = [
    ...
    re.compile(r"tarifa de limpieza|tasas|impuestos", re.I),
]
    ...
    with closing(sqlite3.connect(db_path)) as conn:
        ...
        try:
            cur.execute(
                ...
                WHERE plataforma = 'Booking' AND (esta_activa = 1 OR esta_activa = TRUE OR esta_activa IS NULL)
                LIMIT ?
                """,
                (limit,),
            )
...
def url_variants(base, check_in, check_out):
    common = "group_adults=2&no_rooms=1&selected_currency=USD"
    v1 = f"{base}?checkin={check_in}&checkout={check_out}&{common}"
    v2 = f"{base}?check_in={check_in}&check_out={check_out}&{common}"
    return [v1, v2]
...
def main():
    urls = get_booking_urls(limit=3)
    headless = os.getenv("HEADLESS", "true").lower() == "true"
    results = run_exploration(urls, nights=2, headless=headless)

    out_path = os.path.join("docs_v3", "RESULTADOS_EXPLORACION_BOOKING.md")
    with open(out_path, "w", encoding="utf-8") as f:
        f.write("# Resultados de Exploración Booking (Automático)\n\n")
        for r in results:
            f.write(f"## Listing\n")
